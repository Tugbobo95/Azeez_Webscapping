...
def fetch_ibdb_data():
    ...
    try:
        ...
        for show_item in listings:
            ...
            poster = logo_info.find("div", class_="xt-logo-img").find("img")['src']
            show_name = logo_info.find("div", class_="title").find("div").find("h3").text.strip()
            show_category = left_panel.find("div", attrs={"data-id": "part-b"})\
            ...
            venue_name = venue_blocks[1].find("a").text.strip()
            show_date = venue_blocks[1].find("i").text.strip()

            if (show_name, show_date) in existing_keys:
                continue
            ...
            scraped_results.append({
                ...
                "Date": show_date,
                "Image Link": poster,
                "Theatre Name": venue_name,
                "Show Type": show_category,
                "Detail_Link": show_url,
            })

            print(f"Scraped: {show_name} - {show_date}")
